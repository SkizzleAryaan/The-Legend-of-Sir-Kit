...
#define TXFIFOSIZE   16         // size of the FIFOs (must be power of 2)
#define RXFIFOSIZE   16         // size of the FIFOs (must be power of 2)
#define TXFIFOSUCCESS 1         // return value on success
#define TXFIFOFAIL    0         // return value on failure
#define RXFIFOSUCCESS 1         // return value on success
#define RXFIFOFAIL    0         // return value on failure
...
char volatile *RxPutPt; // put next
char volatile *RxGetPt; // get next
...
uint32_t RxFifo_Size(void){
  if(RxPutPt < RxGetPt){
    return ((uint32_t)(RxPutPt-RxGetPt+(RXFIFOSIZE*sizeof(char)))/sizeof(char));
  }
  return ((uint32_t)(RxPutPt-RxGetPt)/sizeof(char));
}
// Initialize UART0
