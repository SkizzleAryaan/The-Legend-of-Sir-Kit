...
void Board_Init(void){            
  ...
  GPIO_PORTF_DIR_R &= ~0x11; // 5) make PF0 and PF4 in (built-in buttons)
...
//------------Board_Output------------
// Output to LEDs
// Input: bits 3,2,1 have new data to output
// Output: none
...
uint32_t Switch_Input3(void){ 
  return GPIO_PORTB_DATA_R&0x02;    // 0x02 if pressed, 0x00 if not pressed
}
