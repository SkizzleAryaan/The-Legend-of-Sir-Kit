...
void EdgeCounter_Init(void){                          
  ...
  GPIO_PORTE_DIR_R &= ~0x04;    // (c) make PE3 in (built-in button)
  GPIO_PORTE_AFSEL_R &= ~0x04;  //     disable alt funct on PF4
  GPIO_PORTE_DEN_R |= 0x04;     //     enable digital I/O on PF4   
  GPIO_PORTE_PCTL_R &= ~0x0000F000; // configure PF4 as GPIO
  ...
  GPIO_PORTE_IEV_R &= ~0x04;    //     PF4 falling edge event
  GPIO_PORTE_ICR_R = 0x04;      // (e) clear flag4
  GPIO_PORTE_IM_R |= 0x04;      // (f) arm interrupt on PF4 *** No IME bit as mentioned in Book ***
