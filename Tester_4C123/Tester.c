...
/*  outmask = 0x01;              // bit mask of digital output being tested
  ...
  if((digitalA == 0xFC) && (digitalB == 0xFF) && (digitalC == 0xF0) && (digitalD == 0xCF) && (digitalE == 0x3F) && (digitalF == 0x1F)){
    ...
    UART_OutString("This program needs the LM4F120/TM4C123 tester\r\n");
    UART_OutString("Booster Pack to be correctly connected to your\r\n");
    UART_OutString("LaunchPad board.  If the LED is on, then it is\r\n");
    UART_OutString("plugged in backwards.  Unplug the USB cable from\r\n");
    UART_OutString("your computer and plug the Booster Pack the other\r\n");
    UART_OutString("way.  The text on the Booster Pack should be going\r\n");
    UART_OutString("the same way as the text on the LaunchPad.\r\n");
  }
//  if(NYI){
